...
| 구분 | 기술 |
|------|------|
| 백엔드 | Java 17, Spring Boot 4, Spring Web MVC, Spring Data JPA, Spring Security, Bean Validation, SpringDoc OpenAPI 3(Swagger UI) |
...
│   │   └── config/                     # Security, CORS, JWT, 업로드 경로, OpenApiConfig(Swagger)
...
백엔드 기동 후 브라우저에서 다음 주소로 **대화형 API 문서**를 열 수 있습니다(기본 API 포트 `3000` 기준).

| URL | 설명 |
|-----|------|
| `http://localhost:3000/swagger-ui/index.html` | Swagger UI(HTML, Try it out·Authorize) |
| `http://localhost:3000/v3/api-docs` | OpenAPI 3 스펙 **원본 JSON** (도구·Swagger UI가 읽음) |

`/v3/api-docs` 는 브라우저에 긴 JSON만 보이는 것이 **정상**입니다. 문서 화면으로 보려면 위 **Swagger UI** 주소를 쓰세요.

보호된 엔드포인트는 UI 상단 **Authorize**에 `Bearer {액세스토큰}` 형식으로 넣습니다(로그인·회원가입 응답의 `accessToken`). 설정·보안 예외·운영 시 끄는 방법은 [STUDY.md](STUDY.md)의 Swagger 절을 참고하세요.

Docker로 띄운 경우 호스트·포트 매핑에 맞게 `localhost`와 포트만 바꿉니다.
